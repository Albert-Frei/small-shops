...
const GalleryImage = props => {
  ...
  return (
    <div className={css.thumbnailWrapper}>
      <AspectRatioWrapper width={1} height={1}>
        <img className={css.thumbnailImg} src={img.attributes.variants['square-small'].url} alt='' />
      </AspectRatioWrapper>
      <button className={css.removeButton} type='button' onClick={handleRemove}>
        ×
      </button>
    </div>
  );
...
const ProfileGalleryEditor = props => {
  const { className, images, onImageUpload, onRemoveImage, uploadError } = props;
